...
def test_copy_simple(started_cluster):
    ...
    instance.query("CREATE TABLE hits_all ON CLUSTER cluster0 (d UInt64) ENGINE=Distributed(cluster0, default, hits, intHash64(d))")
    instance.query("CREATE TABLE hits_all ON CLUSTER cluster1 (d UInt64) ENGINE=Distributed(cluster1, default, hits, cityHash64(d))")
    instance.query("INSERT INTO hits_all SELECT * FROM system.numbers LIMIT 1000")
    ...
    for instance_name, instance in cluster.instances.iteritems():
        cmd = ['/usr/bin/clickhouse', 'copier', '--config', '/etc/clickhouse-server/config-preprocessed.xml', '--task-path', '/clickhouse-copier/task_simple']

        container = instance.get_docker_handle()
        exec_id = docker_api.exec_create(container.id, cmd, privileged=True, user='root', stderr=True)
    ...
    assert TSV(cluster.instances['s0_0_0'].query("SELECT count() FROM hits_all")) == TSV("1000\n")
    assert TSV(cluster.instances['s1_0_0'].query("SELECT count() FROM hits_all")) == TSV("1000\n")

    assert TSV(cluster.instances['s1_0_0'].query("SELECT DISTINCT cityHash64(d) % 2 FROM hits")) == TSV("0\n")
    assert TSV(cluster.instances['s1_1_0'].query("SELECT DISTINCT cityHash64(d) % 2 FROM hits")) == TSV("1\n")
